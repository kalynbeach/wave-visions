...
export default function MediaDevices() {
  ...
  const setSelectedMediaDevice = (device: MediaDeviceInfo) => {
    ...
    // setMediaDevices({ ...mediaDevices, device: device })
  }
  ...
  return (
    ...
    //           {/* { mediaDevices.device ? mediaDevices.device.label : 'No device selected' } */}
    //         </span>
    ...
    //               className={`p-4 flex flex-row justify-between border rounded cursor-pointer transition hover:border-neutral-800 ${device.deviceId === mediaDevices.device?.deviceId ? 'bg-neutral-900 border-neutral-800' : 'border-neutral-900'}`}
    //             >
    //               <span className={`text-sm font-mono transition ${device.deviceId === mediaDevices.device?.deviceId ? 'text-kb-green' : ''}`}>
    //                 {device.label}
    //               </span>
